...
% check the number of levels and their names for all dimensions
b = all(cellfun(@(x,y) length(x)==length(y) && all(x==y), lvls{1}, lvls{2}));
if ~b, flag=-2; return, end
