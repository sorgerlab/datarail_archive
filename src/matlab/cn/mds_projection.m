...
for i=1:length(LigFam_list)
    ...
    if length(idx)>2 && ~strcmp(LigFam_list{i},'misc')
        ...
        h(i) = plot(MDS_Lig(idx(k),1), MDS_Lig(idx(k),2), '.-','color',Ligcolors(i,:),'linewidth',2,...
            'markersize',20);
    	plot(MDS_Lig(idx,1), MDS_Lig(idx,2), '.','color',Ligcolors(i,:),...
            'markersize',20);
    elseif  ~strcmp(LigFam_list{i},'misc')        
    	h(i) = plot(MDS_Lig(idx,1), MDS_Lig(idx,2), '.-','color',Ligcolors(i,:),...
